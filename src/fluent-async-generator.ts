 ...
 */
export function toGroupedGenerator<T>(sortedSource: () => AsyncGenerator<T>, groupBy: string): () => AsyncGenerator<GroupedByKey<T>> {
...
export function mapGenerator<T, U>(source: () => AsyncGenerator<T>, func: (i: T) => U): () => AsyncGenerator<U> {
    return async function* () {
...
export function batchGenerator<T>(source: () => AsyncGenerator<T>, size: number): () => AsyncGenerator<T[]> {
    return async function* () {
        let batch: T[] = [];
        for await (const item of source()) {
...
export function filterGenerator<T>(source: () => AsyncGenerator<T>, predicate: (p: T) => boolean): () => AsyncGenerator<T> {
    return async function* () {
        for await (const item of source()) {
            if (predicate(item)) {
                yield item;
